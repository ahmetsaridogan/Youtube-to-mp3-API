...
define("DOWNLOAD_FOLDER_PUBLIC", "http://youtube-mp3.test/download/");
...
if(isset($_GET["youtubelink"]) && !empty($_GET["youtubelink"]))
{
	...
	try 
	{
		...
		echo json_encode(array("error" => false, "url" => $video->getUrl(), "title" => $video->getTitle(), "duration" => $video->getDuration(), "file" => $file));
	}
	catch (Exception $e)
	{
